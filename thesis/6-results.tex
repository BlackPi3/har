...
\begin{table}[!t]
\centering
\caption{Validation set performance (Macro F1) for all scenarios. Best results per dataset are in \textbf{bold}; second-best are \underline{underlined}. $\Delta$ indicates change relative to Scenario 2 baseline.}
\label{tab:main-results}
\begin{tabular}{lccccc}
\hline
& \multicolumn{2}{c}{\textbf{UTD-MHAD}} & \multicolumn{2}{c}{\textbf{MM-Fit}} \\
\textbf{Scenario} & F1 & $\Delta$ & F1 & $\Delta$ \\
\hline
\multicolumn{5}{l}{\textit{Baseline}} \\
SC2: Joint Training & 0.682 & --- & \underline{0.885} & --- \\
\hline
\multicolumn{5}{l}{\textit{Loss Ablations}} \\
SC22: No MSE ($\gamma=0$) & 0.082 & $-$88.0\% & --- & --- \\
\hline
\multicolumn{5}{l}{\textit{Architecture Variants}} \\
SC23: Separate Classifiers & 0.674 & $-$1.2\% & \textbf{0.896} & $+$1.2\% \\
SC24: Separate Encoders & 0.678 & $-$0.6\% & 0.862 & $-$2.6\% \\
\hline
\multicolumn{5}{l}{\textit{Auxiliary Data}} \\
SC3: +NTU Pose Data & \textbf{0.691} & $+$1.3\% & 0.848 & $-$4.2\% \\
\hline
\multicolumn{5}{l}{\textit{Adversarial Training}} \\
SC42: Signal Discriminator & \underline{0.671} & $-$1.6\% & --- & --- \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Regression loss is critical:} Scenario 22 (no MSE loss, $\gamma=0$) catastrophically fails with F1 dropping from 0.682 to 0.082 on UTD-MHAD---an 88\% decrease. This demonstrates that the regressor requires direct signal supervision; classification gradients alone are insufficient to guide meaningful simulation.

    \item \textbf{Dataset characteristics determine optimal scenario:} The best-performing scenario differs between datasets:
    \begin{itemize}
        \item \textbf{UTD-MHAD:} Scenario 3 (auxiliary NTU data) achieves the highest F1 of 0.691
        \item \textbf{MM-Fit:} Scenario 23 (separate classifiers) achieves the highest F1 of 0.896
    \end{itemize}
    ...
    \item \textbf{Adversarial training provides modest benefit:} Signal-level discrimination (SC42) maintains competitive performance with only 1.6\% degradation on UTD-MHAD, suggesting domain alignment is achievable without explicit adversarial training when using shared architectures with feature similarity loss.
\end{enumerate}
...
\begin{tabular}{lccc}
\hline
\textbf{Configuration} & \textbf{UTD-MHAD F1} & \textbf{Change} \\
\hline
SC2: Full loss ($\gamma > 0$) & 0.682 & --- \\
SC22: No MSE ($\gamma = 0$) & 0.082 & $-$88.0\% \\
\hline
\end{tabular}
\end{table}
...
\begin{tabular}{lccc}
\hline
\textbf{Configuration} & \textbf{UTD-MHAD} & \textbf{MM-Fit} & \textbf{Avg. Change} \\
\hline
SC2: Shared $F$ & 0.682 & 0.885 & --- \\
SC24: Separate $F$, $F_{\text{sim}}$ & 0.678 & 0.862 & $-$1.6\% \\
\hline
\end{tabular}
\end{table}
...
\hline
SC2: Shared $C$ & 0.682 & 0.885 & Domain-agnostic decisions \\
SC23: Separate $C$, $C_{\text{sim}}$ & 0.674 & \textbf{0.896} & Domain-specific decisions \\
\hline
\end{tabular}
\end{table}
...
Scenario 3 incorporates NTU RGB+D as a secondary pose-only dataset to provide additional training signal for the regressor.
...
\hline
& \multicolumn{2}{c}{\textbf{UTD-MHAD}} & \multicolumn{2}{c}{\textbf{MM-Fit}} \\
\textbf{Configuration} & F1 & Change & F1 & Change \\
\hline
SC2: No auxiliary & 0.682 & --- & 0.885 & --- \\
SC3: With NTU & \textbf{0.691} & $+$1.3\% & 0.848 & $-$4.2\% \\
\hline
\end{tabular}
\end{table}
...
Scenario 42 implements signal-level adversarial training where a discriminator attempts to distinguish real from simulated accelerometer signals.
...
\begin{table}[!t]
\centering
\caption{Signal-level adversarial training (SC42) compared to baseline.}
\label{tab:adversarial-results}
\begin{tabular}{lccc}
\hline
\textbf{Configuration} & \textbf{UTD-MHAD F1} & \textbf{Change} \\
\hline
SC2: No adversarial & 0.682 & --- \\
SC42: Signal discriminator & 0.671 & $-$1.6\% \\
\hline
\end{tabular}
\end{table}
...
The best SC42 configuration uses:
\begin{itemize}
...
SC2 & 0.5 & 50 & 1 & 0.02 & 8 \\
SC3 & 0.1 & 10 & 10 & 0.04 & 8 \\
SC23 & 5.0 & 50 & 1 & 0.02 & 8 \\
SC24 & 5.0 & 100 & 1 & 0.02 & 8 \\
SC42 & 2.0 & 50 & 2 & 0.04 & 16 \\
\hline
\multicolumn{6}{l}{\textit{MM-Fit}} \\
SC2 & 1.0 & 1 & 2 & 0.15 & 48 \\
SC3 & 2.0 & 2 & 2 & 0.15 & 64 \\
SC23 & 1.0 & 1 & 2 & 0.15 & 48 \\
SC24 & 0.5 & 1 & 2 & 0.15 & 48 \\
\hline
\end{tabular}
\end{table}
...
\hline
UTD & SC2 & 50 & 0.5 & Strong feature alignment \\
UTD & SC3 & 1 & 0.01 & Balanced, low classification \\
UTD & SC24 & 100 & 5 & Very strong alignment \\
\hline
MM-Fit & SC2 & 0.5 & 0.5 & Balanced \\
MM-Fit & SC23 & 0.5 & 0.5 & Balanced \\
\hline
\end{tabular}
\end{table}
...
\hline
SC2: Baseline & 0.682 & 0.885 & Strong baseline; shared weights provide implicit domain alignment \\
\hline
SC22: No MSE & 0.082 & --- & Catastrophic failure confirms regression loss is essential \\
\hline
SC23: Sep. Classifiers & 0.674 ($-$1.2\%) & \textbf{0.896} ($+$1.2\%) & Benefits large datasets; hurts small ones due to parameter splitting \\
\hline
SC24: Sep. Encoders & 0.678 ($-$0.6\%) & 0.862 ($-$2.6\%) & Consistently worse; loses implicit alignment from shared weights \\
\hline
SC3: Auxiliary & \textbf{0.691} ($+$1.3\%) & 0.848 ($-$4.2\%) & Domain compatibility critical; helps when auxiliary matches primary \\
\hline
SC42: Adversarial & 0.671 ($-$1.6\%) & --- & Marginal impact; possibly redundant with feature similarity loss \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Joint training is effective:} The baseline scenario (SC2) achieves strong performance on both datasets, validating the core approach of simultaneous regressor and classifier optimization.
