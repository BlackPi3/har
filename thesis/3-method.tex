...
While most other methods consider sensor data simulation and training a HAR classifier as different, independent steps \cite{kwon2020imutube, fortes2021translating, santhalingam2023synthetic}, ours performs both in an end-to-end fashion \cite{zolfaghari2024sensor}.
...
\begin{figure}[t]
  ...
  \caption{Steps in which 3D pose sequence is extracted from frames. This figure has been created by SciSpace.}
...
\end{equation}
where CE denotes cross-entropy loss and $\cos(\cdot, \cdot)$ denotes cosine similarity. This signal-level supervision ensures the regressor learns the pose-to-accelerometer mapping in a physically meaningful way. The similarity loss encourages $F$ to produce similar representations for corresponding real and simulated samples, reducing the domain gap. Crucially, $\mathbf{z}_{\text{real}}$ and $\mathbf{z}_{\text{sim}}$ correspond to the same time window: they originate from synchronized pose and accelerometer recordings of the same activity instance. This pairing is essential---if we compared features from unrelated samples, the loss would push the feature extractor toward a trivial solution where all inputs map to similar representations regardless of the underlying activity. By enforcing similarity only between corresponding pairs, the loss encourages consistent representations for the same motion captured through different modalities, while preserving discriminative information across different activities.
...
We refer to this baseline configuration as the \textbf{Scenario 2.1}. As illustrated in \Cref{fig:baseline}, both the real and simulated paths share the same feature extractor $F$ and classifier $C$, with all three loss terms active ($\alpha, \beta, \gamma > 0$). This serves as the foundation for all experimental variants explored in this thesis. Different configurations selectively enable or disable loss terms, or modify the architecture, to study their individual contributions. All loss weights ($\alpha, \beta, \gamma$) and optimization details are specified in \Cref{chap:implementation}.

\section{Adversarial Training}
\label{sec:adversarial}

Joint training (Scenario 2.1) improves over the two-step approach by allowing classification gradients to reach the regressor. However, a domain gap between real and simulated accelerometer data persists: the feature extractor $F$ may still learn to distinguish between the two sources, limiting the benefit of synthetic augmentation. The main contribution of this thesis is to investigate whether adversarial training can further close this gap and improve HAR performance.

Adversarial training introduces a discriminator network that learns to distinguish real from simulated data. By training the main network to fool the discriminator, we encourage either the feature representations or the generated signals to become domain-invariant. We explore two complementary strategies: a feature-level discriminator that operates on learned embeddings (\Cref{sec:scenario-disc-feature}), and a signal-level discriminator that operates directly on raw accelerometer waveforms (\Cref{sec:scenario-disc-signal}).
...
Beyond adversarial training, we investigate additional architectural choices and loss ablations to understand which components of the baseline are essential. Each variant modifies the baseline configuration in a specific way, enabling controlled ablation studies and architectural comparisons.

\Cref{tab:scenarios} summarizes all scenarios. Scenarios 1 and 2.1 were introduced in \Cref{sec:problem}, and the adversarial scenarios (4.1 and 4.2) in \Cref{sec:adversarial}. The remaining variants are described below.
