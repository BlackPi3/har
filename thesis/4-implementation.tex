...
The feature extractor $F$ transforms raw accelerometer signals into fixed-dimensional feature representations suitable for classification. We implement a temporal convolutional architecture that captures local patterns in the accelerometer signal while maintaining computational efficiency.

\subsubsection{Architecture}

The feature extractor consists of multiple 1D convolutional blocks followed by global pooling and a dense projection layer. Each convolutional block applies:
\begin{enumerate}
    \item 1D convolution with configurable kernel size and filter count
    ...
    \item Max pooling with stride 2 for temporal downsampling
\end{enumerate}

The architecture processes input accelerometer signals $\mathbf{a} \in \mathbb{R}^{3 \times T}$ (3 axes over $T$ timesteps) through successive convolutional layers that progressively expand the channel dimension while reducing the temporal dimension. After the final convolutional block, global average pooling aggregates temporal information, and a fully-connected layer projects to the embedding dimension $d$.

Formally, for a three-layer configuration:
\begin{align}
    \mathbf{h}_1 &= \text{Pool}(\text{Dropout}(\text{LeakyReLU}(\text{BN}(\text{Conv}_1(\mathbf{a}))))) \\
    \mathbf{h}_2 &= \text{Pool}(\text{Dropout}(\text{LeakyReLU}(\text{BN}(\text{Conv}_2(\mathbf{h}_1))))) \\
    \mathbf{h}_3 &= \text{Pool}(\text{Dropout}(\text{LeakyReLU}(\text{BN}(\text{Conv}_3(\mathbf{h}_2))))) \\
    \mathbf{z} &= \text{FC}(\text{GlobalPool}(\mathbf{h}_3))
\end{align}
...
\hline
\texttt{n\_filters} & 9 & Base number of convolutional filters \\
\texttt{kernel\_size} & 5 & Kernel size for convolutions \\
\texttt{embedding\_dim} & 100 & Output feature dimension $d$ \\
\texttt{dropout} & 0.3 & Dropout probability \\
\texttt{pool\_size} & 2 & Max pooling kernel size \\
\texttt{num\_layers} & 3 & Number of convolutional blocks \\
\hline
\end{tabular}
\end{table}
...
The signal-level discriminator $D_S: \mathbb{R}^{3 \times T} \rightarrow [0, 1]$ operates directly on accelerometer signals:
\begin{equation}
    D_S(\mathbf{a}) = \sigma(\text{FC}(\text{Flatten}(\text{CNN}(\text{GRL}(\mathbf{a})))))
\end{equation}
...
The Gradient Reversal Layer (GRL) \citep{ganin2016domain} enables end-to-end adversarial training by modifying gradient flow:
\begin{equation}
...
To stabilize adversarial training, we employ:
\begin{itemize}
