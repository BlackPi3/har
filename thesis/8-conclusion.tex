...
We adapted adversarial domain adaptation techniques to bridge the gap between simulated and real accelerometer data. By incorporating a discriminator with gradient reversal layer, the framework learns domain-invariant representations that generalize across the sim-to-real divide.

Our analysis of discriminator dynamics, feature space visualization, and ablation studies provides insights into when and how adversarial training benefits HAR:
\begin{itemize}
    \item Feature-level discrimination encourages the feature extractor to produce representations indistinguishable between domains
    \item Signal-level discrimination pressures the regressor to produce realistic accelerometer waveforms
    \item The GRL lambda schedule is critical for balancing feature learning and domain alignment
\end{itemize}
...
Adversarial training with feature-level discrimination can provide additional benefit beyond explicit feature similarity loss, but requires careful hyperparameter tuning. Key findings include:
\begin{itemize}
    \item \textbf{Training stability:} Adversarial training is more sensitive to hyperparameters than non-adversarial approaches
    \item \textbf{GRL scheduling:} The lambda schedule is critical for balancing feature learning and domain alignment
    \item \textbf{Feature vs. signal level:} Feature-level discrimination directly improves representations used for classification
\end{itemize}
