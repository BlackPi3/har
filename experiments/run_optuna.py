#!/usr/bin/env python3
"""
Optuna-based Hyperparameter Optimization orchestrator (study-level).

Each Optuna trial launches the single-run entrypoint
`experiments/run_trial.py` with Hydra overrides and reads its
...
Example usage:
  ...
  python experiments/run_optuna.py --n-trials 10 --metric val_f1 --direction maximize \
...
RUN_TRIAL = REPO_ROOT / "experiments" / "run_trial.py"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run Optuna HPO by invoking run_trial.py per trial")
    p.add_argument("overrides", nargs=argparse.REMAINDER, help="Hydra overrides passed through to run_trial.py (e.g., data=mmfit_debug trainer.epochs=5)")
...
def run_single(cfg_overrides: List[str], trial_dir: Path) -> Tuple[bool, Dict[str, Any]]:
    """Invoke run_trial.py with overrides; return (success, metrics_dict)."""
    trial_dir.mkdir(parents=True, exist_ok=True)
    # Force Hydra to use a fixed trial dir to simplify retrieval
    hydra_override = f"hydra.run.dir={trial_dir.as_posix()}"
    cmd = [sys.executable, str(RUN_TRIAL)] + cfg_overrides + [hydra_override]
    try:
