...
def gendata(data_path,
    ...
    for filename in os.listdir(data_path):
        if filename in ignored_samples or filename == '.DS_Store':
            continue
    ...
    #               dtype=np.float32)
    fp = [None] * len(sample_name)

    for i, s in enumerate(tqdm(sample_name)):
        # shape (3, frames, joints, persons)
        data = read_xyz(os.path.join(data_path, s),
                        max_body=max_body_kinect,
                        num_joint=num_joint)
        # CHANGE
        # fp[i, :, 0:data.shape[1], :, :] = data
        assert data[:, :, :, 0].sum() != 0 # check if the first body is empty
        fp[i] = data[:, :, :, 0] # shape (3, frames, joints)

    fp = pre_normalization(fp)
    for i, s in enumerate(fp):
        f = os.path.join(out_path, sample_name[i].split('.')[0] + '.npy')
        np.save(f, s)
...
if __name__ == '__main__':
    ...
    skel_data_path = os.path.join(os.getcwd(), 'data/ntu/orig_skel')
    parser.add_argument('--data_path',
                        default=skel_data_path)
    
    missing_skel_file = os.path.join(os.getcwd(), 'data/ntu/samples_with_missing_skeletons.txt')
    parser.add_argument(
        '--ignored_sample_path',
        default=missing_skel_file)
    
    output_path = os.path.join(os.getcwd(), 'data/ntu/new_skel')
