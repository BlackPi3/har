...
class MMFit(Dataset):
    def __init__(self, pose_file, acc_file, labels_file, sensor_window_length):
        """
        ...
        self.pose = torch.as_tensor(
            utils.load_modality(pose_file)[
                :, :, [0, 1, 16, 17, 18]], dtype=config.dtype
...
for w_id in config.TRAIN_W_IDS + config.VAL_W_IDS + config.TEST_W_IDS:
    ...
    # if config.load_data == config.LoadData.REAL or config.load_data == config.LoadData.COMBINED:
    # Simulated Accelerometer
    # if config.load_data == config.LoadData.SIM or config.load_data == config.LoadData.COMBINED:
    #     if w_id in config.TRAIN_W_IDS:
    #         sim_acc_file = os.path.join(id_dir, w_id + '_' + config.sim_acc_file)
    #         simdataset = MMFit(pose_file=pose_file, acc_file=sim_acc_file,
    #                        labels_file=labels_file, sensor_window_length=config.sensor_window_length)
    #         train.append(simdataset)
    # elif w_id in config.VAL_W_IDS:
    #     val.append(simdataset)
    # elif w_id in config.TEST_W_IDS:
    #     test.append(dataset)
